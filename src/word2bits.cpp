...
void *TrainModelThread(void *id) {
  ...
  while (1) {
	...
	for (c = 0; c < layer1_size; c++) {
	  v[c + l2] += (1.0 - sparsification / c) * (g * context_avg[c] - 2*alpha*reg*v[c + l2]) - sparsification / c * sigmoid(v[c + l2]) * (1.0 - sigmoid(v[c + l2]));
	}
      }
      // hidden -> in
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
	  ...
	  if (last_word == -1) continue;
	  for (c = 0; c < layer1_size; c++) {
	    u[c + last_word * layer1_size] += (1.0 - sparsification / c) * (context_avge[c] - 2*alpha*reg*u[c+last_word*layer1_size]) - sparsification / c * sigmoid(u[c+last_word*layer1_size]) * (1.0 - sigmoid(u[c+last_word*layer1_size]));
	  }
	}
    }
    sentence_position++;
