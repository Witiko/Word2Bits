...
void *TrainModelThread(void *id) {
  ...
  while (1) {
	...
	for (c = 0; c < layer1_size; c++) {
	  ...
	  f += context_avg[c] * cur_val * pow(1.0 - sparsification, c + 1);
	...
	for (c = 0; c < layer1_size; c++) {
	  v[c + l2] += alpha * (pow(1.0 - sparsification, c + 1) * (g * context_avg[c] - 2*reg*v[c + l2]) - (1.0 - pow(1.0 - sparsification, c + 1)));
	}
      }
      // hidden -> in
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
	  ...
	  if (last_word == -1) continue;
	  for (c = 0; c < layer1_size; c++) {
	    u[c + last_word * layer1_size] += alpha * (pow(1.0 - sparsification, c + 1) * (context_avge[c] - 2*reg*u[c+last_word*layer1_size]) - (1.0 - pow(1.0 - sparsification, c + 1)));
	  }
	}
    }
    sentence_position++;
